...
In order to estimate $\theta=Prob(x > 4)$ given $x \sim N_d(0, 1)$ it is possible to generate n number of samples from the normal distribution and check which portion is over 4. This is done by using 100000 samples. This is implemented in R in the following way:

```{R}

#Estimate probability by monte carlo itegration
generate_monte_carlo_samples = function(n) {
  x = sample_standard_normal(n)
  samples = rep(NA, n)
...
n = 100000
x = generate_monte_carlo_samples(n)
est = mean(x)
...
It is possible to estimate $\theta=Prob(x > 4)$ given $x \sim N_d(0, 1)$ it is possible to use importance sampling for variance reduction.

$$
\hat{\mu}_{IS} = \frac{1}{n} \sum h(x_i) w_i.
$$

In this task sampling from the equation below will be used:
$$
  g(x) = \begin{cases}
  ...
  \end{cases}
$$
The normalizing constant $c$ is calculated to be $e^8$ by setting the boundaries of the integral of g(x) from 4 to infinity to 1.
$$
1 = \int_4^\inf c g(x)dx \\
=>c = e^8
$$


To sample from g(x) inversion sample is used. The inverse of the cdf is

$$
X = \sqrt{16-2ln(1-u)}
$$
where u is an uniform variable between 0 and 1.

Then the weighted sample w(x) * 1_{x>=4} from g(x) has to be calculated.

g(x) is sampled, where all samples from g(x) > 4 is set to 1 while the samples below is set to 0. Next, the samples has to be weighed according to the weight function:

$$
w(x)= \frac{f(x)}{g(x)} = \frac{1}{c \sqrt{2 \pi} x}
$$

When $n$ weighted samples from g(x) is created, the mean of these samples will be the expected value of $\theta=Prob(x > 4)$ given $x \sim N_d(0, 1)$.
...
The confidence interval of the estimation with importance sampling is much more precise with the same number of samples at it has more samples in the region of interest. To obtail the same precision by using Mont Carlo Integration one would need ... samples.


## Part 3a)

An implementation of the algorithm presented in C2 with anthithetic variables is implemented in R the following way:
...
One can see that the confidence intervals are approxomatily the same for both functions, even though half the uniform variables are created.
...
$$
a rejection algorithm is created. $f(\theta \lvert \mathbf{y})$ is sampled using $U(0, 1)$. This is implemented in R the following way:
...
Estimate the posterior mean of $\theta$ by Monte-Carlo integration using $n = 10000$ samples from $f(\theta | y)$:
...
The expected number random numbers the rejection sampling algorithm has to create to obtain one sample from $f(\theta | y)$ is $c \geq f(x)/g(x)$. $c$ is chosen as small as possible for the algorthm to be as efficient as possible, resulting in $c = max f(x)/g(x)$. $g(x)=U(0,1) = 1$, meaning $c = max f(x)$.
```{R}
