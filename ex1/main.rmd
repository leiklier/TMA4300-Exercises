...
generate_weighted_samples = function(n) {
  samples = rep(NA, n)
  sample_u = runif(n)
  sample_x = sqrt(16-2*log(1-sample_u)) #Inverse sampling from G
...
cat("Interval width:", confidence_interval[1]-confidence_interval[0])
...
generate_weighted_samples_anth = function(n) {
  samples = rep(NA)
  samples_u_1 = runif(n)
  samples_u_2 = 1 - samples_u_1
  samples_u = c(samples_u_1, samples_u_2)
  sample_x = sqrt(16-2*log(1-samples_u)) #Inverse
  samples[ sample_x <= 4.] = 0
  samples[ sample_x > 4.] = 1/(exp(8)*sqrt(2*pi)*sample_x[ sample_x > 4.])
...
Konfidensintervallet blir smalere

Fordi man får dobbelt så mange datapunkter
