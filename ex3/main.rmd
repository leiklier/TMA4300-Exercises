...
x0 = matrix(x[indices.matrix], nrow=B, ncol=2)
...
bootstrapped.beta_LS = matrix(0, nrow = length(beta_LS), ncol = B)
bootstrapped.beta_LA = matrix(0, nrow = length(beta_LA), ncol = B)
...
for (i in (1:B)) {
  ...
  x.bootstrap_LS = ARp.filter(x0, beta_LS, resample_residual.LS)[3:(T+2)]
  x.bootstrap_LA = ARp.filter(x0, beta_LA, resample_residual.LA)[3:(T+2)]
...
for (i in (1:B)) {
  ...
  # New part of code, calculating corresponding residuals
  boot.e101.LS = ARp.resid(x.bootstrap_LS, beta_LS)
  boot.e101.LA = ARp.resid(x.bootstrap_LA, beta_LA)

  # Choose random residual
  random_residual = sample(1:length(boot.e101.LS),1)
  e101.LS = boot.e101.LS[random_residual]
  e101.LA = boot.e101.LA[random_residual]
  # Find x101
  x101.LS[i] = beta_LS[1] * x[T] + beta_LS[2] * x[T-1] + e101.LS
  x101.LA[i] = beta_LA[1] * x[T] + beta_LA[2] * x[T-1] + e101.LA
  
}

# Calculate quantiles
quantile_LS = quantile(x101.LS, c(0.025, 0.975))
quantile_LA = quantile(x101.LA, c(0.025, 0.975))
...
where $I(\cdot)$ is the indicator function. This means that we know that we only observe the largest value in the pair $(x_i, y_i)$, and we know whether the observed value is $x_i$ or $y_i$. Based on the observed $(z_i, u_i), i=1,\dots, n$ we will use the EM algorithm to find the maximum likelihood estimates for $(\lambda_0, \lambda_1)$. 
